...
### Phase 8 — Frontend + /agent/report endpoint + docs — 2026-04-24

**Decided:**
- **Path A for the agent/frontend wiring: a live `POST /agent/report` endpoint in the FastAPI backend**, not a pre-recorded report shown behind glass. Rationale: the whole portfolio framing is "LLM grounded in measured physical data via segmentation + cited emissions." Hiding the LLM undercuts the story. Pre-build budget headroom (~$9.95) easily covers live demo usage (~$0.02/run), and the backend already proves that `InferenceEngine` can coexist with arbitrary imports in the lifespan handler.
- **Request shape for `/agent/report`: Option 2 (two-phase, stateless).** Client calls `/classify` first, keeps the result in frontend state, then POSTs `{percentages, emissions, total_area_ha, image_label, query}` to `/agent/report`. No re-upload, no server-side session cache, follow-ups on the same image cost nothing in re-inference. Rejected Option 1 (one-shot upload-and-analyze) because it would re-classify on every follow-up query, and rejected a server-side cache keyed on a short hash because stateful backends open failure modes that a stateless design avoids entirely.
- **Non-streaming response for Phase 8.** A real SSE streaming implementation would need a new `run_streaming` method on `ClaudeAgent`, event schema, FastAPI `StreamingResponse`, frontend `EventSource` — estimate 1–2h, high risk of breaking the thing that just worked. Instead we fake 90% of the drama: render the `tool_calls` log from the returned `AgentReport` with a CSS fade-in per entry, plus a 5-step progress ticker that advances every 3s during the ~15s wall-clock. Looks live, isn't live, ships today. Real streaming is P2 in `HACKATHON_TODO.md`.
- **Handler is sync `def`, not `async def`.** `agent.run()` calls the Anthropic client synchronously (blocking HTTP, 10–25s). FastAPI runs sync handlers in a threadpool, keeping the event loop responsive for `/health` polling and other requests. An `async def` handler would stall every other in-flight request for the duration of the agent call.
- **Graceful degradation when `ANTHROPIC_API_KEY` is missing.** Lifespan tries to instantiate `ClaudeAgent` only if the env var is set; if missing, `app.state.agent = None` and `/agent/report` returns HTTP 503 with a clear message. Other endpoints stay unaffected. `/health` surfaces `agent_available: true|false` so the frontend can render a status pill.
- **`max_turns` override in the request is honored via a transient `ClaudeAgent` instance** that reuses the same `client` as the shared one. No mutating the shared agent's fields mid-request (concurrency hazard even with queue-depth-0).
- **`_rehydrate_emissions_result` helper** reconstructs the `EmissionsResult` dataclass from the pydantic `EmissionsReport` the frontend echoes back. Sources are dropped (pydantic-layer only) and no recomputation happens — the frontend-echoed numbers are trusted because they came from our own `/classify` response.
- **Frontend is served by FastAPI at `/ui`** via `app.mount(..., StaticFiles(html=True))`. `GET /` redirects to `/ui/` when `frontend/index.html` exists, else falls back to the original JSON meta response. This is the single biggest UX win of Phase 8 — "clone, set API key, run one command, see demo" actually holds because everything runs same-origin on port 8000 with no separate frontend server and no CORS issues.
- **`load_dotenv(.env)` added to `backend/main.py`** at import time, mirroring `scripts/agent_repl.py`'s pattern, so a teammate launching uvicorn from repo root picks up the key without pre-exporting it.
- **Frontend stack: vanilla HTML/CSS/JS, no build step.** Single `index.html` (~620 lines, CSS-heavy) + `app.js` (~540 lines with comments, ~200 net logic). Two Google Fonts (Fraunces for serif body, JetBrains Mono for chrome). Aesthetic direction: "instrument panel" — dark forest-green (`#0d1410`), burnt-amber accent (`#d4742a`), JetBrains Mono for all numeric displays and UI chrome, Fraunces for the LLM report prose. Deliberately avoids the Inter / Roboto / Space Grotesk defaults and the purple-on-white gradient trap.
- **Class palette in the frontend mirrors `backend/inference.py` PALETTE byte-for-byte** (forest green, crimson building, grey road, dodger-blue water, tan barren, gold agriculture, light-grey background). The composition-bar segments and legend swatches use the same RGB values as the colored mask image, so the two visual representations tell the same chromatic story.
- **Progress stepper with 5 fake steps** (Composing query → Surveying composition → Estimating emissions → Considering interventions → Writing report) advancing on a 3s timer. The last step parks until the response lands, then all steps flip to ✓ with a brief hold before fading out.
- **Inline markdown renderer** (~60 lines) tuned to exactly what Haiku emits in our prompt: `## h2`, `### h3`, `**bold**`, `*em*`, `- lists`, `` `code` ``, `[SRC-N]` and `[SRC-1, SRC-2]` citation chips. Not a general markdown parser. Dry-run against the live `final_text` before shipping. Added `---` horizontal-rule handling post-hoc after Haiku emitted a stray leading separator.
- **Tool calls grouped by turn in the meta footer**, so two parallel calls on the same turn read as "evaluated two scenarios" rather than a bug. Observed in live runs: the agent sometimes fires two `simulate_intervention` calls in one turn to compare candidate interventions before writing the recommendation.
- **Race-safe frontend via a monotonic `gen` token.** Every `loadFile()` call bumps `state.gen`. Every async write-back to state or UI rechecks the token after `await` and bails silently if a newer load has superseded it. Same guard applies to `runAgent()` so image-swap-during-report is handled. Fix was necessary after the live demo surfaced a classic async-overtake-by-one bug (see Surprises).
- **Demo buttons fetch from `/ui/demos/3546.png` and `/ui/demos/2523.png`.** Requires copying the two LoveDA val tiles into `frontend/demos/` after clone; documented in README.
- **`GET /health` now includes `agent_available`** alongside `status` and `device`. Frontend polls every 30s and paints three status pills in the top bar.
- **`GET /` redirects to `/ui/` when the frontend exists** and otherwise returns the old meta JSON. This lets `curl http://localhost:8000` still work for API-only users.
- **ASCII architecture diagram in README.md**, not Mermaid or a separate image file. Matches the project's terminal aesthetic, survives any markdown viewer, no GitHub-rendering dependency.
- **HACKATHON_TODO.md uses strict P0→P3 tiering** with time estimates, files-touched lists, and explicit "definition of done" per task. Includes a "Non-goals" section and a day-of triage timeline (9am–5pm). P0.1 is the map picker — explicitly flagged as the highest-leverage feature with a loud warning against Sentinel-2 at 10m/pixel.

**Rejected alternatives:**
- **Path B (no live agent endpoint, pre-recorded reports in frontend).** Safer, cheaper, less impressive, and dishonest about what the system does. The portfolio framing demands live reasoning.
- **Server-side session cache** keyed on a short hash of the classify result, letting the frontend send only `{id, query}`. Rejected — stateful backend opens LRU-eviction edge cases (what if a follow-up query arrives after eviction?) that a stateless design avoids entirely. Payload size is small enough (~5 KB) to echo.
- **Exposing raw `class_pixel_counts` + `total_pixels` in ClassifyResponse** and reconstructing EmissionsResult server-side from pixels. Purer but touches three files (`inference.py`, `models.py`, `main.py`). The rehydration helper in `main.py` is 15 lines and touches one file — better trade for a 3h phase.
- **React + Vite for the frontend.** The vanilla JS is ~200 net lines and plays nicely with the no-build-step deployment. React would give the hackathon team a better long-term scaffold; it's in HACKATHON_TODO as P2.2 with guidance to only migrate if adding P1 features that benefit from component isolation.
- **Real SSE streaming.** See Decided above for the full rejection rationale. P2.1 in HACKATHON_TODO.
- **A separate frontend HTTP server (`python -m http.server`).** Would mean teammate runs three commands (backend, frontend, browser) instead of one, and re-introduces CORS. Mounting `/ui` on the FastAPI app eliminates both problems.
- **Using a chat-style interface for Phase 8.** Single-shot report per image keeps the UI focused. Chat mode is P1.3 in HACKATHON_TODO (requires backend-side session state, which we deliberately avoided for Phase 8).
- **Mermaid diagram in the README.** Nice when GitHub renders it, degrades to raw text elsewhere. ASCII wins on portability and matches the project's visual language.
- **Favicon.** Browser requests it, gets a 404, nobody cares. Noted as noise in the uvicorn log and left alone.

**Surprises / learnings:**
- **The agent + composition-panel desync bug** was a classic JS async race. User clicked Demo · 2523, then (before `/classify` finished) clicked Demo · 3546. Both classify calls fired. The second response arrived last and painted 3546's composition to the UI. But the user hit Generate in the interval between the two responses, and `state.classify` was captured as 2523's data — producing a report that correctly described 2523 while the UI showed 3546. Fix was a monotonic `gen` token: every `loadFile` increments it, every `await`-resumption rechecks, stale results get silently dropped. Backend request log nailed the diagnosis (both `/classify` POSTs visible, report fired between them). Lesson: every async handler that writes to shared state needs an "is this still the current request?" guard.
- **The Haiku stray-`---`-before-first-heading tic.** On one live run the model opened its response with a literal `---` separator before the `## Sustainability Report` heading. My original markdown renderer treated it as a paragraph and rendered a stray `---` at the top of the report. Fix: detect `/^(-{3,}|\*{3,}|={3,})\s*$/` and emit `<hr />` — with the specific case of a *leading* rule being dropped entirely (only emit `<hr />` if `out.length > 0`). Dry-run against three cases (leading rule, mid-doc rule, no rule) passes. Low-impact but worth catching because an otherwise polished demo shouldn't start with visual noise.
- **`ClassifyResult` doesn't expose `class_pixel_counts` or `total_pixels`.** Phase 6 threw them away internally after computing the `EmissionsResult`. Would have been cleaner to expose them for the agent endpoint to rebuild state from, but the rehydration approach (frontend echoes back the pydantic `EmissionsReport`, backend inflates back to dataclass) turned out to be 15 lines and worked cleanly. Noted as a refactor target if a future phase wants a purer stateless endpoint.
- **Live `/agent/report` wall-clock is ~14s on 3546** across four POST calls to api.anthropic.com — matches Phase 7's 4-turn pattern. Token usage: 13,888 in / 1,006 out, zero cached (expected; Haiku 4.5's 4,096-token cache floor still in effect). Cost per demo run: ~$0.017 on Haiku pricing.
- **Report quality on the live run was outstanding.** On 3546 the agent produced section structure (headline → composition → recommendation → caveats) without explicit format scaffolding, correctly identified the parcel as a net sink, computed a 42-year payback for 0.35 ha of agriculture→forest, explicitly rejected the more aggressive recommend_mitigation top pick in favor of the realistic candidate, and surfaced the forest-IoU-0.39 caveat in its Model Caveats section. The three-pillar framing (measurement → grounding → reasoning) is landing exactly as designed.
- **Backend reload needed for `/ui` mount changes, but not for frontend-only changes.** The static files are served from disk on each request, so HTML/CSS/JS edits show up on Ctrl+Shift+R without a uvicorn restart. Only Python changes require the restart. Worth knowing for future polish work.
- **Hot-reload during development needs `pkill -f uvicorn` + relaunch.** Backgrounding uvicorn with `&` and launching a second one immediately causes a port-already-in-use error; the old one doesn't die cleanly on SIGKILL without the full command-line match. `pkill -f 'uvicorn backend.main:app'` works reliably. Documented in the troubleshooting section.
- **Total Phase 8 API spend: ~$0.20** across one initial smoke test (`/agent/report` against 3546), a race-condition debugging session (~3 extra agent calls), and a successful live demo run. Pre-build budget posture at Phase 8 close: **~$9.75 of $10 remaining**. Sunday buffer fully untouched across the entire Weekend 2.

**Phase 8 closed inside budget.** Estimate was 3h + ~2h Sunday buffer; actual was ~3.5h (frontend took longer than expected due to the race bug surfacing in live testing, and docs added ~45 min). All phase-plan deliverables shipped: `/agent/report` endpoint, single-page frontend with upload/preview/analyze/results, README with architecture diagram, HACKATHON_TODO with tiered P0–P3 tasks. Weekend 2 used ~3 of 4h in buffer across the whole weekend (Phase 6 + 7 closed under estimate, Phase 8 slightly over).

**Open questions carried forward (to hackathon):**
- **Map picker + live tile provider choice** (P0.1 in HACKATHON_TODO). NAIP via USGS/Esri is first choice for resolution fit (~0.6m vs LoveDA's 0.3m). Sentinel-2 at 10m/pixel is a silent-failure trap.
- **Demo-image shortlist beyond 3546 and 2523.** Phase 5's log flagged pre-running inference on 10–20 candidate val tiles and ranking. Still pending; P0.2 in HACKATHON_TODO.
- **Streaming response upgrade.** Real SSE would replace the fake progress stepper. Non-trivial but contained; P2.1 in HACKATHON_TODO.
- **Chat-mode follow-up queries.** The stateless `/agent/report` endpoint means every query is fresh. A `POST /agent/continue` endpoint with in-memory session cache would enable true follow-up conversations. P1.3 in HACKATHON_TODO.
- **Prompt caching activation** by padding the system prompt past 4,096 tokens. Becomes worthwhile if demo volume spikes during the hackathon; not needed for queue-depth-0 usage.
- **More agent tools** (compare_to_region, estimate_payback, get_class_confidence). Design space open; P1.2 in HACKATHON_TODO with candidate list.
